...
def invoke_agent(agent_id, agent_alias_id, session_id, prompt):
    try:
        client = boto3.session.Session().client(service_name="bedrock-agent-runtime", region_name='eu-north-1b')               #1 Add Region Name
        ...
        response = client.invoke_agent(
            agentId='XWLYY094DA',                     #2 Add Agent ID
            agentAliasId='SQBPPDSI6L',                #3 Add Agent Alias ID
            enableTrace=True,
